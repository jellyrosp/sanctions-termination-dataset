...
db_path   = BASE_DIR / "data" / "ist_sanctions.db"
...
# ---------------------------------------------------------------------------
# 2. Date normalisation  DD.MM.YYYY → YYYY-MM
# ---------------------------------------------------------------------------
...
def build_lookup_and_junction(
    ...
    lookup_df  columns: [id_col, "decod"]
    ...
    for uid in sorted(all_ids):
        decod = decod_map.get(uid, uid) if decod_map else uid
...
target_lookup, case_target = build_lookup_and_junction(
    df, "caseid", "target", "target_id"
)
target_lookup["decod"] = target_lookup["target_id"]

# ── Outcome ──────────────────────────────────────────────────────────────────
outcome_lookup, case_outcome = build_lookup_and_junction(
    df, "caseid", "outcome", "outcome_type"
)
# outcome_lookup column should be "outcome_type" + "decod"
# rename to match ERD: PK = outcome_type [TEXT]
outcome_lookup = outcome_lookup.rename(columns={"outcome_type": "outcome_type"})

# ── Multilateralism ───────────────────────────────────────────────────────────
multilat_lookup, case_multilat = build_lookup_and_junction(
    df, "caseid", "multilateralism", "multilateralism_id"
)
multilat_lookup["decod"] = multilat_lookup["multilateralism_id"]

# ── Dataset ───────────────────────────────────────────────────────────────────
dataset_lookup, case_dataset = build_lookup_and_junction(
    df, "caseid", "datasets", "dataset_id"
)
dataset_lookup["decod"] = dataset_lookup["dataset_id"]
...
CREATE TABLE IF NOT EXISTS Case_Outcome (
    case_id      TEXT REFERENCES Case(case_id),
...
def insert(df: pd.DataFrame, table: str, con: sqlite3.Connection):
    """Insert-or-ignore all rows of a dataframe into a table."""
    if df.empty:
        return
...
con = sqlite3.connect(db_path)
tables = [
    "Case", "Outcome", "Sender", "Target", "Measure",
